...
-record(kz_binding, {binding :: ne_binary() | '_'
                     ...
                     ,binding_responders = queue:new() :: queues:queue() | '_'
                      %% queue -> [#kz_responder{}]
...
-type filter_updates() :: [{ne_binary(), {pos_integer(), queues:queue()}}] | [].
...
-spec map_responders(list(), map_responder_fun(), queues:queue()) -> list().
map_responders(Acc, Map, Responders) ->
    [catch(Map(Responder))
