...
handle(Data, Call) ->
    case  wh_json:get_ne_value(<<"mode">>, Data) of
        ...
        <<"voicemail">> -> handle_voicemail(Data, Call);
        _ -> cf_exe:stop(Call)
    end.
...
handle_uma(Data, Call) ->
    ...
    case ss7c_if:get_subscriber_info(<<"msisdn">>, Msisdn, rest) of
        {ok, Props} ->
            RespJObj = wh_json:from_list(Props),
...
handle_sms(_Data, Call) ->
    CaptureGroup = whapps_call:kvs_fetch('cf_capture_group', Call),
    To = <<$+, CaptureGroup/binary>>,
    From = whapps_call:from(Call),
    Name = whapps_call:caller_id_name(Call),
    Body = <<From/binary, " is trying to reach you via the Altilink secure UMA service">>,
    Props = [{<<"from">>, From}
             ,{<<"to">>, To}
             ,{<<"body">>, Body}
             ,{<<"proto">>,rest}],
    ss7c_if:submit_sms(Props),
    cf_exe:stop(Call).

handle_voicemail(_Data, Call) ->
    CaptureGroup = whapps_call:kvs_fetch('cf_capture_group', Call),
    Msisdn = <<$+, CaptureGroup/binary>>,
    ...
    case ss7c_if:get_subscriber_info(<<"msisdn">>, Msisdn, rest) of
        {ok, Props} ->
            RespJObj = wh_json:from_list(Props),
            FTN = wh_json:get_ne_value([<<"isd">>,<<"ftns">>,<<"cfnrc">>], RespJObj),
            NewCall = whapps_call:set_request(<<FTN/binary, "@", Realm/binary>>, Call),
            NewCall2 = whapps_call:set_to(<<Msisdn/binary, "@", Realm/binary>>, NewCall),
            NewFlow = wh_json:from_list([{<<"data">>,<<>>},{<<"module">>,<<"resources">>},{<<"children">>,<<>>}]),
%            Diversion = list_to_binary(["<sip:", whapps_call:request(Call), ">", ";reason=no-answer"]),
%            lager:info("add diversion as ~s", [Diversion]),
%            NewCall2 = whapps_call:set_custom_sip_header(<<"Diversions">>, Diversion, NewCall),
%            NewFlow2 = wh_json:set_value(<<"custom_sip_headers">>, wh_json:from_list([{<<"Diversions">>, Diversion}]), NewFlow),
            lager:info("call forwarding to ~s", [FTN]),
            branch(NewFlow, NewCall2);
        _Else ->
            lager:info("failed to secure number ~s", [CaptureGroup]),
            cf_exe:stop(Call)
    end.
...
maybe_bridge_endpoints(Data, Call) ->
    User = wh_json:get_value(<<"imsi">>, Data),
    Realm =  whapps_call:request_realm(Call),
    Req = [{<<"Username">>, User}
           ,{<<"Realm">>, Realm}
           ,{<<"Fields">>, [<<"Contact">>]}
...
bridge_to_endpoints(Data, Call) ->
    Imsi = wh_json:get_value(<<"imsi">>, Data),
    Msisdn = wh_json:get_value(<<"msisdn">>, Data),
