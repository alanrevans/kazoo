...
handle(Data, Call) ->
    ...
    case couch_mgr:get_results(AccountDb, <<"devices/listing_by_msrn">>, ViewOptions) of
        {'ok', []} ->
            lager:error("msrn ~s doesn't exist", [Msrn]),
            cf_exe:stop(Call);
        {'ok', [JObj]} ->
            continue(wh_json:merge_jobjs(Data, JObj), Call);
        {'ok', _} ->
...
continue(Data, Call) ->
    case whapps_util:amqp_pool_request(Data
                              ,fun wapi_hlr:publish_get_cf_info_req/1
                              ,fun wapi_hlr:get_cf_info_req_v/1
                              ,5000
                             ) of
        {ok, RespJObj} ->
            FTN =  wh_json:get_ne_value(<<"ftn">>, RespJObj),
            NewCall = whapps_call:set_request(<<FTN/binary, "@sip.kagesys.com">>, Call),
            NewFlow = wh_json:from_list([{<<"data">>,<<>>},{<<"module">>,<<"resources">>},{<<"children">>,<<>>}]),
            lager:info("call forwarding to ~s", [FTN]),
            branch(NewFlow, NewCall);
        {error, _Err} -> cf_exe:stop(Call)
    end.
