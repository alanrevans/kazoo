...
handle_req(JObj, Props) ->
    ...
    case not is_binary(whapps_call:account_id(Call))
        andalso is_uma_sms(Call)
        andalso resource_allowed(Call)
    of
        'false' -> 'ok';
        'true' ->
            Call1 = update_uma_sms(Call),
            lager:info("received a request asking if doodle can route this message"),
            AllowNoMatch = allow_no_match(Call1),
            case cf_util:lookup_callflow(Call1) of
                ...
                {'ok', Flow, NoMatch} when (not NoMatch) orelse AllowNoMatch ->
                    NewFlow = maybe_prepend_preflow(Call1, Flow),
                    maybe_reply_to_req(JObj, Props, Call1, NewFlow, NoMatch);
                {'ok', _, 'true'} ->
...
update_uma_sms(Call) ->
        Domain = whapps_call:from_realm(Call),
        ...
        case couch_mgr:get_results(?WH_ACCOUNTS_DB, <<"accounts/listing_by_realm">>, ViewOptions) of
            ...
            {'ok', [JObj]} ->
                ...
                lists:foldl(fun(F, C) -> F(JObj, C) end
                                ,Call
                                ,Routines
                               );
            {'error', _E} ->
                lager:debug("error ~p querying realm ~p in accounts database",[_E, Domain]),
                Call
        end.
        
    set_account_id(JObj, Call) ->
        Account_id = wh_json:get_ne_value([<<"value">>,<<"account_id">>], JObj),
        whapps_call:set_account_id(Account_id, Call).
    
    set_account_db(JObj, Call) ->
        Account_db = wh_json:get_ne_value([<<"value">>,<<"account_db">>], JObj),
        whapps_call:set_account_db(Account_db, Call).
        
    set_authorizing_id(_JObj, Call) ->
        whapps_call:set_authorizing_id(<<"kagesys_uma_device">>, Call).
    
    set_authorizing_type(_JObj, Call) ->
        whapps_call:set_authorizing_type(<<"device">>, Call).
